...
```text
주피터 노트북상에서 matplotlib, seaborn이 설치되어 있지 않아 설치하였다.
```

#### 결과 관찰

matplotlib, seaborn이 설치되었고, pandas, pathlib, numpy, matplotlib.pyplot, seaborn 모듈이 import되었다. 또한 각각 pd, Path, np, plt, sns의 축약자로 사용할 수 있다.

#### 나의 해석과 판단

starter scaffold는 자동으로 분석 전에 폴더나 파일을 자동화하여 생성하는 것을 의미한다.

#### 한계와 추가 확인 사항

이번 챕터에서 기본 분석 제안을 제시하고, 주피터노트북 실습환경을 구현했다. 따라서 앞으로 실제 코드들을 실행해보고 제안한 분석을 실행 해보아야 할 것이다.
...
```text
이번 장에서 가장 중요한 것은 데이터 분석에 있어서 어디에 LLM을 써야하고, 어떤 영역은 사람이해야하고, LLM의 답변은 반드시 사람이 검토해야한다는 것이다. 아무리 LLM을 사용한다고 하더라도 최종 의사결정과 분석의 시작점은 사람이 제시해야한다.
```

### LLM을 데이터 분석에 사용할 때 가장 조심해야 할 점

```text
LLM에게 과의존 및 과신해서는 안되고, 민감한 정보를 공유해서는 안된다.
```
...
| --- | --- | --- |
| 질문 정의 | 질문의 구체화 도움 | 질문의 초안 제시, 방향 제시 |
| 데이터 확인 | 사람이 제시한 범주 내에서 확인 및 점검 | 민감한 데이터 hide, LLM 답변 검토 |
| 코드 작성 | 초안 작성 및 오류 메세지 간단한 해석 | 코드 설계 틀 제공 및 LLM의 코드를 활용하여 목적에 맞게 변형 |
| 결과 해석 | 결과 해석 의견 제시 | LLM이 섣불리 결론을 내지 않도록 실제 데이터와 비교하여 진위 검증 및 의견 검토 |
| 최종 판단 |  X | 사람의 온전한 책임의 영역 |
...
```text
LLM 활용 시 분석 목적에 맞게 답변이 나왔는지 검토하고, LLM에서 가치판단이 일어나지 않았는지 주의해야한다.
```

### 아직 확인이 필요한 내용 1개

```text
실제 이 데이터들을 가지고 분석을 한다면 데이터의 양과 분포 확인이 필요하다.
```
